...
dada_forward$`SRR3667040_R2_filtered.fastq`
...
glimpse(merged_ASVs$`SRR3667040_R1_filtered.fastq`)
```
...
INTERPRETATION #1: Hypothesized ASV Length

What is the sequencing strategy?

Paired-end 250 bp (2×250 bp) Illumina MiSeq sequencing was used, targeting the V4 hypervariable region of the 16S rRNA gene with the 515F and 806R primers.


What's the total length of our starting amplicons?

Our primers are named after the position they bind to on the 16S gene. So:
806 - 515 = 291
The total length of the starting amplicon is 291 base pairs, including primers.


What is the expected ASV length without primers?

The 515F primer (GTGYCAGCMGCCGCGGTAA) is 19 base pairs.
The 806R primer (GGACTACNVGGGTWTCTAAT) is 20 base pairs.
291 - 19 - 20 = 252
The expected ASV length without primers is 252 base pairs.
IMPORTANT: Unlike the in-class dataset (which used the Kozich et al., 2013 library prep and did not sequence the primers), our dataset used the standard Illumina 2-step PCR library preparation, meaning the primers were present in the raw sequences. They were subsequently identified and removed using Cutadapt in 01_QualityTrimming.Rmd, leaving us with reads of ~230 bp going into filterAndTrim().


What is the expected ASV length after filterAndTrim()?

After Cutadapt primer removal, our reads were ~230 bp long.
In 01_QualityTrimming.Rmd, we used truncLen = c(228, 225) — trimming 2 bp from the forward read and 5 bp from the reverse read to remove low-quality tails.
Since DADA2 merges forward and reverse reads into a single contiguous ASV, the final ASV length is determined by the amplicon itself (252 bp), not the individual read lengths — as long as there is sufficient overlap.
The expected merged ASV length is therefore approximately 252 bp.


What is the expected overlap between forward and reverse reads?

Our forward reads are 228 bp and our reverse reads are 225 bp after filterAndTrim().
Combined read length: 228 + 225 = 453 bp
Expected amplicon length (without primers): 252 bp
Expected overlap: 453 - 252 = 201 bp
This is a very substantial overlap (~80% of the amplicon length), which is excellent. A minimum of 12–20 bp of overlap is required for DADA2 to merge reads successfully, so 201 bp of overlap gives us a very high confidence in accurate merging and ASV assignment.</span>
...
```{r trim-ASVs}
# Subset only ASVs that are between 400 and 430 bps long
raw_ASV_table_trimmed <- 
  raw_ASV_table[, nchar(colnames(raw_ASV_table)) %in% 400:430]
...
plot_ASVLength_trimmed <- 
  ...
  ggplot(aes(x = Seq_Length)) + 
  geom_histogram(binwidth = 1) +   # binwidth = 1 to show each bp individually
  scale_x_continuous(limits = c(398, 432)) +  # ← zoomed in to your retained range
...
Before running the code below, be sure to copy over the file `/workdir/in_class_data/metadata_physeq.RData` to `Biodiversity_Salinity_16S/data`.

```{r make-metadata}
# ── 1. Load the SRA metadata ──────────────────────────────────────────────────
metadata_df <- read.csv("data/SraRunTable.csv", header = TRUE)

# Intuition check — what columns do we have?
head(metadata_df)
colnames(metadata_df)

# ── 2. Set the Run accession as row names so phyloseq can match samples ────────
row.names(metadata_df) <- metadata_df$Run

# Confirm Run accessions look correct
head(rownames(metadata_df))  # Should look like SRR3666998, SRR3666999, etc.

# ── 3. Diagnose any mismatches with the ASV table ─────────────────────────────
missing_from_ASV <- rownames(metadata_df)[!rownames(metadata_df) %in% colnames(final_ASV_table)]
missing_from_meta <- colnames(final_ASV_table)[!colnames(final_ASV_table) %in% rownames(metadata_df)]

cat("In metadata but NOT in ASV table:", length(missing_from_ASV), "\n")
print(missing_from_ASV)
cat("\nIn ASV table but NOT in metadata:", length(missing_from_meta), "\n")
print(missing_from_meta)
cat("\nTotal metadata samples:", nrow(metadata_df), "\n")
cat("Total ASV table samples:", ncol(final_ASV_table), "\n")
cat("Number of matching samples:", sum(rownames(metadata_df) %in% colnames(final_ASV_table)), "\n")

# ── 4. Subset BOTH objects to only shared samples ─────────────────────────────
shared_samples <- intersect(rownames(metadata_df), colnames(final_ASV_table))

cat("Number of shared samples:", length(shared_samples), "\n")

# Subset metadata to shared samples only
metadata_df <- metadata_df[shared_samples, ]

# Subset ASV table to shared samples only
final_ASV_table <- final_ASV_table[, shared_samples]

# ── 5. Confirm the match is now perfect in both directions ────────────────────
stopifnot(all(rownames(metadata_df) %in% colnames(final_ASV_table)))
stopifnot(all(colnames(final_ASV_table) %in% rownames(metadata_df)))
cat("✓ All metadata sample names match ASV table sample names\n")

# ── 6. Convert to phyloseq sample_data object ─────────────────────────────────
metadata_physeq <- sample_data(metadata_df)
